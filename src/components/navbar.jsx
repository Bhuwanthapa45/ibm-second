...
const Navbar = () => {
  ...
  return (
    <div className='fixed z-[999] w-full px-6 py-4 md:px-[5vw] md:py-[2vh] flex flex-wrap justify-between items-center bg-zinc-900 sm:bg-transparent text-zinc-100'>
      <div className='logo'>
      {/* <svg width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {/* <img src="" alt="" /> */}
        <h1 className='font-semibold'>LearnLoom</h1>
      </div>
      ...
      <div className={`w-full md:flex md:items-center md:w-auto ${isOpen ? 'block' : 'hidden'}`}>
        <div className='links gap-4 md:gap-10 block md:flex pt-[4vh] sm:pt-0'>
          {[
            { name: 'Home', path: '/' },
            ...
            { name: 'LMS', path: '/lms' },
            { name: 'Collabration', path: '/collabration' }
          ].map((item, index) => (
            <Link
              key={index}
              to={item.path}
              className={`text-lg font-semibold capitalize block ${index === 5 && 'ml-0 md:ml-32'} hover:text-[#CDEA68] hover:font-bold`}
            >
              {item.name}
            </Link>
          ))}
